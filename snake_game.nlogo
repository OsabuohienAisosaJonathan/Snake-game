...
;;  Brooks, P. (2020) Snake-simple. Stuyvesant High School. Available
...
to setup ; observer
  ...
  ; there will always be two randomly placed pieces of food within the environment:
  make-food
  make-food
  reset-ticks
end
...
to go ; observer
  ...
  ask snakes [
    ; Set which direction the snake is facing:
    ifelse mode = "random" [
      ...
      ; Add other search methods here as needed
    ]
    ...
    ; Check for a collision (and thus game lost)
    ...
    ; Check if max age reached (and thus game won)
    ...
    ; Set the patch color and age of the snake's head
...
to depth-first-move  ; snake procedure
  ; Get the current patch of the snake
  let current-patch patch-here
  ; Find the nearest food patch using depth-first search
  let path depth-first-search current-patch green []
  ifelse path != [] [
    ; If a path to food patch is found, select a target patch
    let target-patch one-of path
    ; Face towards the target patch
    if target-patch != nobody [
      face target-patch
    ]
  ] [
    ; If no path to food patch is found, continue moving randomly
    face-random-neighboring-patch
  ]
end

to-report depth-first-search [current-patch target-color visited]
  ; Check if the current patch has already been visited
  if member? current-patch visited [
    report []
  ]
  ; Base case: if the current patch has the target color (food), return a list containing only the current patch
  ifelse [pcolor] of current-patch = target-color [
    report (list current-patch)
  ] [
    ; Mark the current patch as visited
    let updated-visited (lput current-patch visited)
    ; Get neighbors that are clear and have not been visited
    let unvisited-neighbors []
    ask neighbors with [not member? self updated-visited and member? pcolor clear-colors] [
      set unvisited-neighbors lput self unvisited-neighbors
    ]
    foreach unvisited-neighbors [
      let neighbor-patch self
      let sub-path (depth-first-search neighbor-patch target-color updated-visited)
      ifelse sub-path != [] [
        ; If a path to the target color is found, return the path including the current patch
        report (list current-patch sub-path)
      ] [
        ; If no path is found, continue iterating over other neighbors
        report []
      ]
    ]

    ; If no path is found from any neighbor, return an empty list
    report []
  ]
end
...
to breadth-first-move  ; snake procedure
  ; Get the current patch of the snake
  let current-patch patch-here
  ; Find the nearest food patch using breadth-first search
  let target-path (breadth-first-search current-patch green)
  ; If a path to a food patch is found, face towards it
  if target-path != [] [
    let next-patch item 1 target-path  ; Get the next patch from the found path
    face next-patch
  ]
  ; If no path to food patch is found, continue moving randomly
  face-random-neighboring-patch
...
to-report breadth-first-search [start end-color]
  ; Initialize a queue with the starting patch as the first path
  let queue (list (list start))
  ; Initialize the list of visited patches with the starting patch
  ...
  while [ not empty? queue ] [
    ...
    ; Remove the dequeued path from the queue
    set queue but-first queue
    ; Get the current patch from the end of the current path
    let current-patch last current-path

    ; Check if the current patch matches the target color
    if [pcolor] of current-patch = end-color [
      ; If the target color is found, return the current path
      report current-path
    ]

    ; Explore neighboring patches of the current patch
    let neighboring-patches (list (patch (pxcor - 1) (pycor)) (patch (pxcor + 1) (pycor)) (patch (pxcor) (pycor - 1)) (patch (pxcor) (pycor + 1)))
    ; Iterate over neighboring patches
    foreach neighboring-patches [
      let next-patch one-of neighboring-patches
      ; Check if the next patch is not visited and is not a wall
      if not member? next-patch visited and [pcolor] of next-patch != wall-color [
        ; Mark the neighbor patch as visited
        set visited lput next-patch visited
        ; Extend the current path to include the neighbor patch
        let new-path lput next-patch current-path
...
to uniform-search-move  ; snake procedure
  ; Get the current patch of the snake
  let current-patch patch-here
  ; Find the nearest food patch using uniform search
  let path uniform-search current-patch green
  ; If a path to food patch is found, select a target patch
...
to-report uniform-search [start end-color]
  let queue (list (list start))  ; Initialize a queue with the starting patch as the first path
  let visited (list start)  ; Initialize the list of visited patches with the starting patch

  while [ not empty? queue ] [
    let current-path first queue  ; Dequeue the first path from the queue
    set queue but-first queue  ; Remove the dequeued path from the queue
    let current-patch last current-path

    if [pcolor] of current-patch = end-color [
      report current-path  ; Return the path if the target color is found
    ]

    let neighboring-patches (list (patch (pxcor - 1)
      (pycor)) (patch (pxcor + 1) (pycor)) (patch (pxcor) (pycor - 1)) (patch (pxcor) (pycor + 1)))
    foreach neighboring-patches [
      let next-patch one-of neighboring-patches
      if not member? next-patch visited and [pcolor] of next-patch != wall-color [
        let new-path lput next-patch current-path  ; Extend the current path to include the neighbor patch
        set queue lput new-path queue  ; Enqueue the new path
        set visited lput next-patch visited  ; Mark the neighbor patch as visited
      ]
    ]
  ]
  report []  ; Return an empty list if no path is found
end


;greedy search algorithm implementation
to greedy-search-move  ; snake procedure
  ; Get the current patch of the snake
  let current-patch patch-here
  ; Find the nearest food patch using greedy search
  let path greedy-search current-patch green []
  ; If a path to a food patch is found, select a target patch
  ifelse path != [] [
    let target-patch one-of path
    ; Face towards the target patch
    if target-patch != nobody [
...
to-report greedy-search [current-patch target-color visited]
  ; Check if the current patch has already been visited
  if member? current-patch visited [
    report []
  ]
  ...
  ifelse [pcolor] of current-patch = target-color [
    ...
    foreach unvisited-neighbors [
      let neighbor-patch self
      let sub-path (greedy-search neighbor-patch target-color updated-visited)
      ifelse sub-path != [] [
        ; If a path to the target color is found, return the path including the current patch
        report (list current-patch sub-path)
      ] [
